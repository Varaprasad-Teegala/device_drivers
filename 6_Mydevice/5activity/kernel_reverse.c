...
ssize_t my_read (struct file *file, char __user *buf, size_t lbuf, loff_t *ppos)
{
	*ppos = 0;
	int l = 0;
	for(int i=0;char_device_buf[i];i++)
		l++;
	for(int i=0;i<l/2;i++)
	{
		t = char_device_buf[i];
		char_device_buf[i] = char_device_buf[l-i-1];
		char_device_buf[l-i-1] = t;
	}
...
{
	printk("\nThis is init\nFile : %s\tFunction : %s\tLine:%d\n",__FILE__,__func__,__LINE__);
	register_chrdev(60,"reverse",&dev_ops);
